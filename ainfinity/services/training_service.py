...
from ainfinity.exceptions import (JobAlreadyExistsException,
                                  JobNotFoundException, SkyPilotException)
from ainfinity.schema.training_job import (EvaluationMetrics, GPUMetrics,
                                           JobInfo, JobStatus,
                                           LaunchJobRequest, ResourceConfig,
...
class SkyPilotService:
    ...
    def _generate_yaml_config(self, request: LaunchJobRequest) -> str:
        ...
        if request.training.per_device_train_batch_size:
            overrides.append(f"training_arguments.per_device_train_batch_size={request.training.per_device_train_batch_size}")
        ...
        if request.resources.memory:
            config["resources"]["memory"] = request.resources.memory
        if request.resources.cpus:
            config["resources"]["cpus"] = request.resources.cpus
    ...
    def launch_job(self, request: LaunchJobRequest) -> JobInfo:
        ...
        try:
            ...
            # Launch the job
            result = self._run_sky_command(cmd)
    ...
    def get_job_logs(self, job_name: str, tail: int = 100) -> str:
        ...
        try:
            ...
            result = self._run_sky_command(
                ["sky", "logs", job_name, "--tail", str(tail)],
                check=False
            )
