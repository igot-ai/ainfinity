...
class TrainingMetrics(BaseModel):
    ...
    samples_per_second: Optional[float] = Field(
        default=None, description="Training throughput"
    )
    last_updated: Optional[datetime] = Field(
        default=None, description="Last metrics update time"
    )
...
class EvaluationMetrics(BaseModel):
    ...
    eval_accuracy: Optional[float] = Field(
        default=None, description="Evaluation accuracy"
    )
    eval_perplexity: Optional[float] = Field(
        default=None, description="Perplexity score"
    )
    eval_samples: Optional[int] = Field(
        default=None, description="Number of evaluation samples"
    )
    last_evaluated: Optional[datetime] = Field(
        default=None, description="Last evaluation time"
    )
...
class GPUMetrics(BaseModel):
    ...
    gpu_utilization: Optional[float] = Field(
        default=None, ge=0, le=100, description="GPU utilization %"
    )
    gpu_memory_used: Optional[float] = Field(
        default=None, description="GPU memory used (GB)"
    )
    gpu_memory_total: Optional[float] = Field(
        default=None, description="Total GPU memory (GB)"
    )
    gpu_temperature: Optional[float] = Field(
        default=None, description="GPU temperature (°C)"
    )
