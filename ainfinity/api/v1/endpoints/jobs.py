...
from ainfinity.schema.training_job import (JobListResponse, JobResponse,
                                           LaunchJobRequest)
...
async def get_job(job_name: str):
    ...
    return JobResponse(
        success=True,
        message=f"Job '{job_name}' status retrieved",
        job_info=job_info
    )
...
async def delete_job(job_name: str):
    ...
    return JSONResponse(content={
        "success": True,
        "message": f"Job '{job_name}' deleted from database"
    })
