...
class TrainingClient:
    ...
    def launch_job(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = requests.post(f"{self.base_url}/api/v1/jobs", json=job_config)
        response.raise_for_status()
        return response.json()
    
    def get_job_status(self, job_name: str) -> Dict[str, Any]:
        """Get status of a job"""
        response = requests.get(f"{self.base_url}/api/v1/jobs/{job_name}")
        response.raise_for_status()
        return response.json()
    ...
    def stop_job(self, job_name: str) -> Dict[str, Any]:
        """Stop a running job"""
        response = requests.delete(f"{self.base_url}/api/v1/jobs/{job_name}")
        response.raise_for_status()
        return response.json()
    
    def get_logs(self, job_name: str, tail: int = 100) -> Dict[str, Any]:
        """Get job logs"""
        response = requests.get(f"{self.base_url}/api/v1/jobs/{job_name}/logs", params={"tail": tail})
        response.raise_for_status()
        return response.json()
    
    def delete_job(self, job_name: str) -> Dict[str, Any]:
        """Delete job from database"""
        response = requests.delete(f"{self.base_url}/api/v1/jobs/{job_name}/delete")
        response.raise_for_status()
        return response.json()
